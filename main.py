            ...
            async with aiohttp.ClientSession() as session:
                ...
                if days_limit > 0:
                    ...
                    if b_delta > 0:
                        ...
                        embed.add_field(name=f"📊 Desempenho ({period_label})", value="Nenhuma partida nova 
async def save_snapshot(account_id: int, battles: int, wins: int, damage: int):
    now = int(time.time())
    ...
    try:
        async with session.get(search_url, timeout=timeout) as resp:
            if resp.status == 200:
                data = await resp.json()
                if data.get("status") == "ok" and data.get("data"):
                    return "na", base_url, data["data"][0]["account_id"], data["data"][0]["nickname"]
    except Exception:
        pass
    return None, None, None, None
...
async def regcla(ctx):
    ...
    base_url = "https://api.wotblitz.com"
    async with aiohttp.ClientSession() as session:
        url_search = f"{base_url}/wotb/clans/list/?application_id={APPLICATION_ID}&search={clean_tag}"
        
        try:
            async with session.get(url_search, timeout=10) as resp:
                data = await resp.json()
                if data.get("status") != "ok" or not data.get("data"):
                    await loading_msg.edit(content=f"❌ A Wargaming não encontrou nenhum clã com a tag `{clean_tag}` no servidor NA. Confirme se a tag está correta nas variáveis do Render.")
                    return
            ...
            if not clan_id:
                await loading_msg.edit(content=f"❌ Clã `{clean_tag}` não foi identificado na listagem.")
                return

            await loading_msg.edit(content=f"🔍 Clã **{clan_name}** encontrado! Baixando membros...")

            url_info = f"{base_url}/wotb/clans/info/?application_id={APPLICATION_ID}&clan_id={clan_id}"
            async with session.get(url_info, timeout=10) as resp:
                info_data = await resp.json()
                if info_data.get("status") != "ok":
                    await loading_msg.edit(content=f"❌ Erro ao consultar informações detalhadas do clã.")
                    return
                    
                members = info_data.get("data", {}).get(str(clan_id), {}).get("members", [])

            if not members:
                await loading_msg.edit(content=f"❌ O clã `{clean_tag}` foi encontrado, mas a lista de membros está vazia ou inacessível.")
                return
            ...
            for i in range(0, len(account_ids), 100):
                batch_ids = account_ids[i:i+100]
                ids_str = ",".join(map(str, batch_ids))
                acc_info_url = f"{base_url}/wotb/account/info/?application_id={APPLICATION_ID}&account_id={ids_str}"
...
async def blitz(ctx):
    ...
    try:
        ...
        if opcao == "1":
            ...
            await ctx.send("Escolha o **período**:\n1️⃣ **24 Horas**\n2️⃣ **7 Dias**\n3️⃣ **30 Dias**\n\n*Digite `1`, `2` ou `3`:*")
            msg_periodo = await bot.wait_for("message", check=check, timeout=90.0)
            days_limit, period_label = {"1": (1, "24 Horas"), "2": (7, "7 Dis_all.get("battles", 0)
                                wins = stats_all.get("wins", 0)
                                damage = stats_all.get("damage_dealt", 0)
...
async def blitz(ctx):
    ...
    try:
        ...
        if opcao == "1":
            ...
            await ctx.send("Escolha o **período**:\n1️⃣ **24 Horas**\n2️⃣ **7 Dias**\n3️⃣ **30 Dias**\n\n*Digite `1`, `2` ou `3`:*")
            msg_periodo = await bot.wait_for("message", check=te**! "
                f"Todos os membros do clã podem utilizar as funções normalmente — desde estatísticas gerais de um jogador, "
                f"lista de tanques jogados até estatísticas de tanques específicos —, tudo de forma totalmente automática, "
                f"**sem precisar cadastrar uma por uma**."
            )

            await loading_msg.edit(content=resposta)

        except Exception as e:
            await loading_msg.edit(content=f"❌ Ocorreu um erro ao sincronizar o clã: {e}")
